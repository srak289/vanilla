...
function setup {
    ...
    if ((Get-Item $PROFILE).linktarget -eq $null) {
        ...
        Remove-Item $PROFILE
        if (!Test-Path) {
            # We cannot make links w/o admin access 
...
function hide_directories {
    Set-ItemProperty -Path $HOME/.local -Name Attributes -Value Hidden
    Set-ItemProperty -Path $HOME/.vanilla -Name Attributes -Value Hidden
    Set-ItemProperty -Path $HOME/.ssh -Name Attributes -Value Hidden
}
